export function initHomeExperience({ onWorkspaceChange } = {}) {
    ...
    function examMockPreview(detail = false) {
      ...
      return `<div class="example-preview exam-feature-visual exam-mock-visual${detail ? ' is-detail' : ''}"><h3 class="exam-visual-heading">Mock Exams</h3><div class="exam-mock-visual-grid">${exams.map(exam => `<section class="exam-mock-visual-card"><span class="exam-mock-visual-icon">${icon(exam.icon)}</span><div class="exam-mock-title-row"><strong>${exam.title}</strong><span class="exam-mock-badge${exam.tone}">${exam.badge}</span></div><p>${exam.copy}</p><p class="exam-mock-meta">${exam.meta}</p><span class="exam-mock-state">Not started</span><span class="exam-mock-button">Start exam →</span></section>`).join('')}</div></div>`;
    }

    function examProgressPreview(detail = false) {
      return `<div class="example-preview exam-feature-visual exam-result-shell${detail ? ' is-detail' : ''}"><div class="exam-result-progress"><i></i></div><section class="exam-result-summary"><h4>FINAL Exam: Biology 101</h4><div class="exam-result-stats"><span>Points: <strong>21 / 26</strong></span><span>Percentage: <strong>81%</strong></span></div><div class="exam-result-analysis"><strong>Final exam analysis</strong><span>You demonstrated strong understanding of cell structure, genetics, and ecology. Review cellular respiration and meiosis before the next assessment.</span></div></section><div class="exam-result-tabs"><strong>All questions</strong><span>21 Correct</span><span>5 Incorrect</span><span>0 Unanswered</span></div><section class="exam-result-question"><small>Multiple Choice · 1/26</small><h5>Where does the electron transport chain occur in a eukaryotic cell?</h5><div class="exam-result-answer wrong">A. Cytoplasm</div><div class="exam-result-answer correct">D. Inner mitochondrial membrane</div><div class="exam-result-feedback"><strong>Feedback</strong><br>The electron transport chain is embedded in the inner mitochondrial membrane, where it creates the proton gradient used to produce ATP.</div></section>${detail ? `<section class="exam-result-question"><small>Short Answer · 2/26</small><h5>State one key difference between mitosis and meiosis.</h5><div class="exam-result-answer wrong">Mitosis makes four genetically different cells.</div><div class="exam-result-answer correct">Mitosis produces two genetically identical diploid cells, while meiosis produces four genetically varied haploid cells.</div></section><div class="exam-result-actions"><span>Retake exam</span><span>Result breakdown</span></div>` : ''}</div>`;
    }
    ...
    function chemistryPreview(example) {
      if (example && example.coverImage) return `<div class="example-preview preview-chemistry cover"><img src="${example.coverImage}" alt="Chemistry structure visualization" /></div>`;
    ...
    function bindDialogInteraction(type, example = null) {
      ...
      const renderChemistryStructure = index => {
        ...
        if (chemistryImage) {
          ...
          chemistryImage.alt = `${structure.title} structure`;
        }
